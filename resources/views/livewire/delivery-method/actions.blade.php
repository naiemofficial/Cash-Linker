<div class="inline-flex items-center">
    ...
    <div class="inline-flex rounded-md">
        @if(!$deliveryMethod->trashed())
            ...
            <a title="Delete" href="{{ route('delivery-method.delete', $deliveryMethod->id) }}/redirect" class="inline-flex items-center justify-center h-[24px] w-[24px] text-[12px] border border-gray-300 rounded-r-sm text-red-600 hover:bg-red-600 hover:text-white hover:border-red-600 focus:outline-none transition ease-in-out duration-150 -ml-px cursor-pointer">
                <i class="fas fa-trash"></i>
            </a>
        @else
            <a href="{{ route('delivery-method.restore', $deliveryMethod->id) }}/redirect" class="inline-flex items-center px-2 py-[2px] text-[12px] border border-gray-300 rounded-l-sm text-blue-500 hover:bg-blue-500 hover:text-white hover:border-blue-500 focus:outline-none transition ease-in-out duration-150 cursor-pointer">
                <i class="fas fa-trash-restore mr-2"></i> Restore
            </a>
            <a href="{{ route('delivery-method.deletePermanent', $deliveryMethod->id) }}/redirect" class="inline-flex items-center px-2 py-[2px] text-[12px] border border-gray-300 rounded-r-sm text-red-600 hover:bg-red-600 hover:text-white hover:border-red-600 focus:outline-none transition ease-in-out duration-150 -ml-px cursor-pointer">
